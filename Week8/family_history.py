...
def print_death_age(people_dict):
    ...
    for item in people_dict.items():
        name = item[1][NAME_INDEX]
        death = item[1][DEATH_YEAR_INDEX]
        print(f'{name}, Died: {death}')
...
def count_genders(people_dict):
    ...
    for item in people_dict.items():
        ...
        if gender == 'F':
            ...
            males += 1
    print(f'# of females: {females}')
    print(f'# of males {males}')
...
def print_marriages(marriages_dict, people_dict):
    ...
    for item in marriages_dict.items():
        wed_year = item[1][WEDDING_YEAR_INDEX]
        
        husband = item[1][HUSBAND_KEY_INDEX]
        husband_age = wed_year - people_dict[husband][BIRTH_YEAR_INDEX]
        husband_name = people_dict[husband][NAME_INDEX]

        wife = item[1][WIFE_KEY_INDEX]
        wife_age = wed_year - people_dict[wife][BIRTH_YEAR_INDEX]
        wife_name = people_dict[wife][NAME_INDEX]

        print(f'{wife_name} {wife_age} > {wed_year} < {husband_name} {husband_age}')
